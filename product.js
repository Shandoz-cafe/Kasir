function loadProducts() {
    ...
    tbody.innerHTML = products.map((p, i) => {
        ...
        return `<tr>
            <td>${i+1}</td>
            <td><span style="background:#e2e8f0; padding:4px 8px; border-radius:5px; font-size:0.85rem; font-weight:600;">${p.category || 'Umum'}</span></td>
...
function addProduct() {
    ...
    const products = JSON.parse(localStorage.getItem('products') || '[]');
    products.push({ id: Date.now().toString(), name, category, cost, price, stock });
    localStorage.setItem('products', JSON.stringify(products));
    
    document.getElementById('prodName').value = ''; document.getElementById('prodCategory').value = '';
